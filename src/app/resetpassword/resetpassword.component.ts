...
export class ResetpasswordComponent implements OnInit {
  ...
  submitResetForm(resetForm: FormGroup) {
    let token:any = localStorage.getItem("userId")
    this.spinner.show();
    if (resetForm.valid) {
      this._AuthService.resetPassword(resetForm.value,token).subscribe({
        next: (response) => {
          if (response.message == 'Reset successfully') {
            this.spinner.hide();
            if (this.language == 'EN') {
              this.toastr.success(
                'Password reset successfully please update your password',
                '',
                { positionClass: 'toast-bottom-right', timeOut: 5000 }
              );
            } else {
              this.toastr.success(
                'تم إعادة تعيين كلمة المرور بنجاح يرجى تحديث كلمة المرور الخاصة بك',
                '',
                { positionClass: 'toast-bottom-right', timeOut: 5000 }
              );
            }
