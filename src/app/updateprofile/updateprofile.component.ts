...
export class UpdateprofileComponent implements OnInit {
  ...
  updateProfileForm: FormGroup = new FormGroup({
    "name": new FormControl(null, [Validators.required,Validators.minLength(3), Validators.maxLength(30), Validators.pattern(/^[a-z A-Z]+$/)]),
  ...
  submitUpdateProfileForm(updateProfileForm: FormGroup) {    
    ...
    if (updateProfileForm.valid) {
      ...
      this._AuthService.updateProfile(_id,token,updateProfileForm.value).subscribe({next:(response) => {
        ...
        if (response.message == "Updated successfully") {
        ...
        }else if (response.message == "Updated successfully verify email") {
          ...
          this.spinner.hide();
          if (this.language == 'EN') {
            this.toastr.success('Profile updated successfully please verify your email and login', "",{positionClass:'toast-bottom-right',timeOut: 5000});

          } else {
            this.toastr.success(
              'تم تحديث الملف الشخصي بنجاح ، يرجى التحقق من بريدك الإلكتروني وتسجيل الدخول',
              '',
              { positionClass: 'toast-bottom-right', timeOut: 5000 }
            );
          }
